...
class LlavaStableLMEpochForCausalLM(StableLMEpochForCausalLM, LlavaMetaForCausalLM):
    ...
    def forward(
        ...
        output_image_features, input_ids, attention_mask, past_key_values, inputs_embeds, labels = self.prepare_inputs_labels_for_multimodal(input_ids, attention_mask, past_key_values, labels, images, clip_resize_list)
        ...
        if labels is not None:
            ...
            if len(task_token_idx) > 0:
                loss_fct = CrossEntropyLoss(reduction='none')
                token_mask = (1 - torch.isin(shift_labels, torch.tensor(task_token_idx).to(shift_labels)).float()).bool()
                loss = loss_fct(shift_logits, shift_labels)
                loss = loss[token_mask].mean()
            else:
    ...
    def prepare_inputs_for_generation(
        self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, images=None, clip_resize_list=None, **kwargs
    ):
        if past_key_values:
